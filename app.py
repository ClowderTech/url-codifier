...
ADMIN_PASSWORD = hash_password(os.environ["ADMIN_PASSWORD"])
...
async def execute_async_code(code: str) -> str:
...
async def create_redirect(
    request: Request, code: str = Form(...), password: str = Form(...)
):
    ...
    if hashed_password != ADMIN_PASSWORD:
        ...
        message = [
            "success",
            Markup(
                f"Redirect successfully created! Your key: <a href='/redirect/{key}'>{key}</a>"
            ),
        ]
    ...
    if document:
        ...
        if code:
            ...
            return (
                ...
                400,
            )  # Bad request error for empty 'code'
    return (
        "Handler function not found.",
        404,
    )  # Not found error if document doesn't exist.
